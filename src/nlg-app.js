...
</iron-iconset-svg>`;document.head.appendChild($_documentContainer$1.content);Polymer({_template:html`
    <style>
      :host {
        display: inline-block;
      }
...
</custom-style>`;document.head.appendChild($_documentContainer$2.content);const $_documentContainer$3=document.createElement("template");$_documentContainer$3.setAttribute("style","display: none;");$_documentContainer$3.innerHTML=`<dom-module id="paper-material-styles">
  <template>
    <style>
      :host, html {
        --paper-material: {
...
</dom-module>`;document.head.appendChild($_documentContainer$3.content);const $_documentContainer$4=document.createElement("template");$_documentContainer$4.setAttribute("style","display: none;");$_documentContainer$4.innerHTML=`<dom-module id="paper-button">
...
</dom-module>`;document.head.appendChild($_documentContainer$4.content);Polymer({is:"paper-button",behaviors:[PaperButtonBehavior],properties:{raised:{type:Boolean,reflectToAttribute:!0,value:!1,observer:"_calculateElevation"}},_calculateElevation:function(){if(!this.raised){this._setElevation(0)}else{PaperButtonBehaviorImpl._calculateElevation.apply(this)}}});const $_documentContainer$5=document.createElement("template");$_documentContainer$5.setAttribute("style","display: none;");$_documentContainer$5.innerHTML=`<custom-style>
  <style is="custom-style">
    html {
...
</custom-style>`;document.head.appendChild($_documentContainer$6.content);const $_documentContainer$7=document.createElement("template");$_documentContainer$7.setAttribute("style","display: none;");$_documentContainer$7.innerHTML=`<dom-module id="paper-checkbox">
  <template strip-whitespace="">
    <style>
      :host {
        display: inline-block;
...
</custom-style>`;document.head.appendChild($_documentContainer$10.content);const PaperInputAddonBehavior={attached:function(){flush$1();this.fire("addon-attached")},update:function(){}};_exports.PaperInputAddonBehavior=PaperInputAddonBehavior;_exports.$paperInputAddonBehavior={PaperInputAddonBehavior:PaperInputAddonBehavior};Polymer({_template:html`
    <style>
      :host {
        display: inline-block;
        float: right;
...
</custom-style>`;document.head.appendChild($_documentContainer$11.content);Polymer({_template:html`
    <style>
      :host {
        display: block;
        padding: 8px 0;
...
</dom-module>`;document.head.appendChild($_documentContainer$13.content);Polymer({_template:html`
...
`,is:"paper-item",behaviors:[PaperItemBehavior]});Polymer({_template:html`
    ...
    <slot></slot>
`,is:"paper-listbox",behaviors:[IronMenuBehavior],hostAttributes:{role:"listbox"}});const unsafeHTML=value=>directive(part=>{const tmp=document.createElement("template");tmp.innerHTML=value;part.setValue(document.importNode(tmp.content,!0))});_exports.unsafeHTML=unsafeHTML;_exports.$unsafeHtml={unsafeHTML:unsafeHTML};const COLORS=[{id:"Keendoo primary blue",value:"#1F6B7E"},{id:"Keendoo seconday blue",value:"#3B98AE"},{id:"Keendoo tertiary blue",value:"#7CC4D1"},{id:"Keendoo error red",value:"#EC2C45"},{id:"Keendoo dev orange",value:"#FE6900"},{id:"Purple",value:"#8400FF"},{id:"Red",value:"#FF0044"},{id:"Orange",value:"#FF9E00"},{id:"Keendoo dark mode",value:"#1D1F24"},{id:"Black",value:"#000000"},{id:"Medium gray",value:"#7F8284"},{id:"Light gray",value:"#BCBFBF"},{id:"White",value:"#FFFFFF"}];_exports.COLORS=COLORS;_exports.$colors={COLORS:COLORS};class NLGColorPicker extends LitElement{_render({label,fill,opacity}){let selected;if(fill){const defaultColor=COLORS.find(color=>color.value.toLowerCase()===fill.toLowerCase());selected=defaultColor?COLORS.indexOf(defaultColor):COLORS.length}return html$2`
    <style>
      div.color-preview {
        width: 20px;
        height: 20px;
...
`}static get properties(){return{fill:String,label:String,opacity:Number}}_renderDefaultColorOptions(){return COLORS.map(color=>html$2`
      <paper-item>
        <div class="color-preview" style$="background-color:${color.value}"></div>
        ${color.id}
      </paper-item>
    `)}_onColorSelected(e){if(e.target.selected===COLORS.length){return}this.fill=COLORS[e.target.selected].value;this._dispatchColorChangedEvent()}_onFillChanged(e){if(!e.target.value){return}this.fill=e.target.value;this._dispatchColorChangedEvent()}_onOpacityChanged(e){if(!e.target.value){return}this.opacity=e.target.value;this._dispatchColorChangedEvent()}_dispatchColorChangedEvent(){this.dispatchEvent(new CustomEvent("color-changed",{detail:{fill:this.fill,opacity:this.opacity}}))}}window.customElements.define("nlg-color-picker",NLGColorPicker);class NLGSizePicker extends LitElement{_render({width,height}){return html$2`
    <style>
      paper-input {
        width: 8em;
        text-align: center;
    ...
    <paper-input id="height" label="Height" value="${height}" type="Number" allowed-pattern="[0-9]" min="1"
      on-value-changed="${e=>this._setHeight(e)}" invalid="${1>height}">
    ...
    <paper-input id="width" label="Width" value="${width}" type="Number" allowed-pattern="[0-9]" min="1"
      on-value-changed="${e=>this._setWidth(e)}" invalid="${1>width}">
      <span slot="prefix"></span>
      <div slot="suffix">px</div>
    </paper-input>
`}static get is(){return"nlg-size-picker"}static get properties(){return{height:Number,width:Number}}_setHeight(e){this.height=e.target.value;this._dispatchSizeChanged()}_setWidth(e){this.width=e.target.value;this._dispatchSizeChanged()}_dispatchSizeChanged(){this.dispatchEvent(new CustomEvent("size-changed",{detail:{width:this.width,height:this.height}}))}}window.customElements.define("nlg-size-picker",NLGSizePicker);const TEMPLATES={"Keendoo-Infinity-1C":{viewBox:`0 0 256 256`,width:256,height:256,primaryFill:"#3B98AE",primaryOpacity:1,backgroundFill:"#FFFFFF",backgroundOpacity:0,geometry:`
    ...
    `},"Keendoo-Infinity-2C":{viewBox:`0 0 256 256`,width:256,height:256,primaryFill:"#3B98AE",primaryOpacity:1,secondaryFill:"#7CC4D1",secondaryOpacity:1,backgroundFill:"#FFFFFF",backgroundOpacity:0,geometry:`
      <g>
        <g>
    ...
    `},"Keendoo-Infinity-3C":{viewBox:`0 0 256 256`,width:256,height:256,primaryFill:"#1F6B7E",primaryOpacity:1,secondaryFill:"#7CC4D1",secondaryOpacity:1,tertiaryFill:"#3B98AE",tertiaryOpacity:1,backgroundFill:"#FFFFFF",backgroundOpacity:0,geometry:`
      <g>
    ...
    `},"Keendoo-3C":{viewBox:`0 0 512 128`,width:512,height:128,primaryFill:"#1D1F24",primaryOpacity:1,secondaryFill:"#1F6B7E",secondaryOpacity:1,tertiaryFill:"#7CC4D1",tertiaryOpacity:1,backgroundFill:"#FFFFFF",backgroundOpacity:0,geometry:`
      <g>
        <g>
          <g>
    ...
    `}};_exports.TEMPLATES=TEMPLATES;const getSkeleton=(template,params)=>{if(!template){return""}const defaultParams=Object.assign({},TEMPLATES[template]);params=Object.assign(defaultParams,params);return`
  ...
  `};_exports.getSkeleton=getSkeleton;_exports.$templates={TEMPLATES:TEMPLATES,getSkeleton:getSkeleton};const DOMURL=window.URL||window.webkitURL||window,renderSVG=(svg,mimetype="image/png")=>{if(!svg.trim()){return}svg=svg.replace(/svg-style>/gmi,"style>");const holder=document.createElement("div");holder.style.display="none";holder.innerHTML=svg;const svgElement=holder.firstElementChild;if(svgElement&&"svg"!=svgElement.nodeName.toLowerCase()){throw new Error("Is not an SVG")}document.body.appendChild(svgElement);svgElement.style.display="";svgElement.style.opacity="1";const w=svgElement.clientWidth,h=svgElement.clientHeight;svgElement.style.display="none";const canvas=document.createElement("canvas");canvas.style.display="none";canvas.width=w*window.devicePixelRatio;canvas.height=h*window.devicePixelRatio;canvas.style.width=w+"px";canvas.style.height=h+"px";const blob=new Blob([svg],{type:"image/svg+xml"}),url=DOMURL.createObjectURL(blob),img=new Image,promise=new Promise((resolve,reject)=>{img.onload=()=>{canvas.getContext("2d").drawImage(img,0,0,canvas.width,canvas.height);DOMURL.revokeObjectURL(url);document.body.removeChild(svgElement);resolve(canvas.toDataURL(mimetype))};img.onerror=()=>{reject("Cannot render SVG")}});img.src=url;return promise};_exports.renderSVG=renderSVG;_exports.$svgRenderer={renderSVG:renderSVG};window.devicePixelRatio=1;class NLGApp extends LitElement{_render({template,width,height,primaryFill,primaryOpacity,secondaryFill,secondaryOpacity,tertiaryFill,tertiaryOpacity,backgroundFill,backgroundOpacity,_showAlphaLayer}){const content=getSkeleton(template,{width,height,primaryFill,primaryOpacity,secondaryFill,secondaryOpacity,tertiaryFill,tertiaryOpacity,backgroundFill,backgroundOpacity});return html$2`
    <style>
      :host {
        display: flex;
        flex-direction: column;
...
`}static get properties(){return{width:Number,height:Number,primaryFill:String,primaryOpacity:Number,secondaryFill:String,secondaryOpacity:Number,tertiaryFill:String,tertiaryOpacity:Number,backgroundFill:String,backgroundOpacity:Number,template:String,_showAlphaLayer:Boolean}}_drawTemplateOptions(){return Object.keys(TEMPLATES).map(id=>html$2`
      <paper-item id="${id}">${id}</paper-item>
    `)}_drawColorPicker(label,fill,opacity,onChanged){return!fill?null:html$2`
      <nlg-color-picker label="${label}" fill="${fill}" opacity="${opacity}" on-color-changed="${e=>onChanged(e)}"></nlg-color-picker>
    `}_onTemplateSelected(e){this.template=e.detail.item.id;const template=TEMPLATES[this.template];this.width=template.width;this.height=template.height;this.primaryFill=template.primaryFill;this.primaryOpacity=template.primaryOpacity;this.secondaryFill=template.secondaryFill;this.secondaryOpacity=template.secondaryOpacity;this.tertiaryFill=template.tertiaryFill;this.tertiaryOpacity=template.tertiaryOpacity;this.backgroundFill=template.backgroundFill;this.backgroundOpacity=template.backgroundOpacity}_set(propertyName,value){if(value){this[propertyName]=value}return this[propertyName]}_onSizeChanged(e){this._set("width",e.detail.width);this._set("height",e.detail.height)}_onPrimaryChanged(e){this._set("primaryFill",e.detail.fill);this._set("primaryOpacity",e.detail.opacity)}_onSecondaryChanged(e){this._set("secondaryFill",e.detail.fill);this._set("secondaryOpacity",e.detail.opacity)}_onTertiaryChanged(e){this._set("tertiaryFill",e.detail.fill);this._set("tertiaryOpacity",e.detail.opacity)}_onBackgroundChanged(e){this._set("backgroundFill",e.detail.fill);this._set("backgroundOpacity",e.detail.opacity)}_onShowLayerChanged(e){this._showAlphaLayer=e.target.checked}_download(svg){renderSVG(svg).then(url=>{const link=document.createElement("a");link.setAttribute("hidden",!0);link.download=`${this.template}-${this.height}x${this.width}`;link.href=url;link.click()})}}window.customElements.define("nlg-app",NLGApp)});